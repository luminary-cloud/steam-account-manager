...
public:
    ...
    // Load from disk if the file exists. Silently leaves the store empty on
    // missing or malformed files (the index is non-critical).
    void load();

    // Append + persist. If the store would exceed kMaxEvents, the oldest
    // acknowledged events are pruned first; oldest unacked if no acked exist.
    ...
    // Read-only access for the panel's Recent changes list.
