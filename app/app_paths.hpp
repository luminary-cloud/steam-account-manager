...
// Snapshot of a picked userdata/<id> folder, holding its appid folders only.
...
// Settings::run_as_admin, read straight off disk. wWinMain decides whether to self-elevate
// before it can build an AppState, so this is the one setting with a pre-AppState reader.
// Defaults to true when the file is missing or unreadable.
